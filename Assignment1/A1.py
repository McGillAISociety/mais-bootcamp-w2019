...
get_ipython().run_cell_magic('time', '', 'X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25)')


# In[7]:


get_ipython().run_cell_magic('time', '', 'print(X_train.shape, X_test.shape)')
...
get_ipython().run_cell_magic('time', '', '# Instantiate the model\nlinear = LinearRegression()\n# Fit the model (train)\nlinear.fit(X_train, y_train_hot)')


# In[11]:


get_ipython().run_cell_magic('time', '', "# use trained model to predict both train and test sets\ny_train_pred = linear.predict(X_train)\ny_test_pred = linear.predict(X_test)\n\n# print accuracies\nprint('train acc: ', accuracy_score(y_train_pred.argmax(axis=1), y_train))\nprint('test acc: ', accuracy_score(y_test_pred.argmax(axis=1), y_test))\nprint(linear.coef_.shape)\n#Why is it 10 rows?")
...
get_ipython().run_cell_magic('time', '', "ridge = Ridge(alpha=1)\nridge.fit(X_train, y_train_hot)\nprint('train acc: ', accuracy_score(ridge.predict(X_train).argmax(axis=1), y_train))\nprint('test acc: ', accuracy_score(ridge.predict(X_test).argmax(axis=1), y_test))")
...
get_ipython().run_cell_magic('time', '', "logreg = LogisticRegression(C=0.01, multi_class='multinomial', solver='saga', tol=0.1)\nlogreg.fit(X_train, y_train)\nprint('train acc: ', accuracy_score(logreg.predict(X_train), y_train))\nprint('test acc: ', accuracy_score(logreg.predict(X_test), y_test))")
...
get_ipython().run_cell_magic('time', '', 'pca = PCA(n_components=0.8, whiten=True)\nX_train_pca = pca.fit_transform(X_train)\nX_test_pca = pca.transform(X_test)')
...
get_ipython().run_cell_magic('time', '', 'X_train_pca.shape')
...
get_ipython().run_cell_magic('time', '', "lsvc = LinearSVC(dual=False, tol=0.01)\nlsvc.fit(X_train_pca, y_train)\nprint('train acc: ', accuracy_score(lsvc.predict(X_train_pca), y_train))\nprint('test acc: ', accuracy_score(lsvc.predict(X_test_pca), y_test))")
...
get_ipython().run_cell_magic('time', '', "psvc = SVC(kernel='poly', degree=3, tol=0.01, cache_size=4000)\n## YOUR CODE HERE - fit the psvc model\npsvc.fit(X_train_pca,y_train)\n## YOUR CODE HERE - print training accuracy\n## YOUR CODE HERE - print test accuracy\nprint('train acc: ', accuracy_score(psvc.predict(X_train_pca), y_train))\nprint('test acc: ', accuracy_score(psvc.predict(X_test_pca), y_test))")
...
get_ipython().run_cell_magic('time', '', "rsvc = SVC(kernel='rbf', tol=0.01, cache_size=4000)\nrsvc.fit(X_train_pca,y_train)\n## YOUR CODE HERE - fit the rsvc model\n## YOUR CODE HERE - print training accuracy\n## YOUR CODE HERE - print test accuracy\nprint('train acc: ', accuracy_score(rsvc.predict(X_train_pca), y_train))\nprint('test acc: ', accuracy_score(rsvc.predict(X_test_pca), y_test))")
