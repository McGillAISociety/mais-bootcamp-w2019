...
# 
# One Hot Encoding: One hot encoding is a process by which categorical variables are converted into a form that could be provided to ML algorithms to do a better job in prediction. As the number of unique entries (categories) increases, the index also proportionally increases. Each unique index is associated with a category.
# 
# Label Encoding: The categorical value assignment can be done using sklearn’s LabelEncoder. The problem with label encoding is that it assumes higher the categorical value, better the category.
# 
...
# 
# Ridge Regression: Find a line that doesn't fit the data as well as linear regression to reduce variance in the long term. We introduce a small amount of bias in our model at the beginning, but in return, we get a significant drop in variance 
...
# It increases the accuracy of the testing
...
# 
# Cross-Validation: divide the dataset into two parts... (train,test)
...
rf = RandomForestClassifier(n_estimators = 200, min_samples_split = 2, max_features = 75)
## YOUR CODE HERE - fit the rf model (just like logistic regression)
rf.fit(X_train, y_train)
## YOUR CODE HERE - print training accuracy
print('training acc: ', accuracy_score(rf.predict(X_train), y_train))
...
# 
# SVM: Separation of classes. That’s what SVM does. It finds out a line/ hyper-plane (in multidimensional space that separate outs classes).
# 
# Kernel Trick: Polynomial and exponential kernels calculates separation line in higher dimension. 
# 
...
nn = MLPClassifier(hidden_layer_sizes=(100,), solver='adam', verbose=False)
