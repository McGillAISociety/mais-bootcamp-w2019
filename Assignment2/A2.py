...
# 
# The root mean squared error went down, which means that the model with 10-degree polynomial is a better fit for the testing data.
...
### YOUR CODE HERE - Plot your the curve on the training data set
plt.scatter(train_x, train_y, s=10)
plt.plot(train_x, y_poly_pred, color='g')
plt.show()
...
print("Mean Squared Error: {}".format(mse))
plt.scatter(valid_x, valid_y, s=10)
plt.plot(valid_x, valid_y_poly_pred, color='g')
plt.show()


# #### How does the mean square error compare to the previous two models? Why do you think this is the case?
# 
# The mean square error were higher than both previous models. As can be seen the curve on the testing set, the 20-degree polynomial is not a good fit with a lot of data scattering. The model is too closely fitting for the training set particularly but does not fit the testing data, explained by overfitting.
...
def phi (data, degree):
    matrix = np.zeros((len(data), degree + 1)) #array of 11: 1, x, x^2,...x^10
    for counter,x in enumerate(data):
        for i in range(degree + 1):
            matrix[counter][i] = x ** i
    return matrix
...
x_valid = phi(x_test,10)
y_test_pred = np.matmul(W, np.transpose(x_valid))
...
def g(w, X, y):
    return sum([x_i * (np.matmul(np.transpose(w), x_i) - y_i) for x_i, y_i in zip(X, y)])
def grad_dec(X, y):
    w0 = [1,0]
    ...
    for i in range(0, 1000): #time out after 1000 iterations
        ...
        if abs(mean_squared_error(y_train, np.matmul(X, w)) - mean_squared_error(y_train, np.matmul(X, w0))) < epsilon:
            return w
        w0 = w


w = grad_dec(X,y_train)
