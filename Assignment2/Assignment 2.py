...
# 
# It goes down, and this is because a 10-degree polynomial has sufficient complexity to predict the data.
...
# 
# The mean square error is much higher. This is because the model is too good at capturing the training data and it does not generalize.
...
def phi(X):
    ...
    for i in range(1, 10 + 1):
        np.power(X, i)
        res = np.column_stack((res, np.power(X, i)))
    return res
...
print(phi_x.shape)
W = np.matmul(
        np.linalg.inv(np.matmul(phi_x.transpose(), phi_x)),
        np.matmul(phi_x.transpose(), Y_train))
### YOUR CODE HERE - Make predictions on the training set and calculate the root mean squared error. Plot the results.
Y_pred = np.matmul(W.transpose(), phi(X_train).transpose()).transpose()
print("Training RMSE: " + str(np.sqrt(np.mean(np.power(Y_pred - Y_train,2)))))
### YOUR CODE HERE - Make predictions on the validation set and calculate the mean squared error. Plot the results.
Y_pred = np.matmul(W.transpose(), phi(valid_x).transpose()).transpose()
print("Validation MSE: " + str(np.mean(np.power(Y_pred - valid_y, 2))))
plt.scatter(valid_x, valid_y, s=10)
plt.plot(valid_x, Y_pred, color='r')
plt.show()
