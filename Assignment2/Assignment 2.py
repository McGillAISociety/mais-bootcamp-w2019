...
def calculate_weight(x_matrix, y_matrix):
    ...
    W = np.matmul(left_inverse, right_side)
    return W
...
pred_train = np.matmul(W, x_matrix_transpose)
...
pred_valid = np.matmul(weight, np.transpose(x_matrix_valid))
...
def gradient_descent_algorithm(data, rot = 0.7):
    y = np.array(data[1])
    x = np.array(data[0])
    
    iterations = 0
    
    b = random.random() * random.randint(0, sys.maxsize)
    m = random.random() * random.randint(0, sys.maxsize)
...
predicted_y_train = gradient_descent_algorithm(train2)
plt.figure()
...
predicted_y_test = gradient_descent_algorithm(test2)
plt.figure()
...
predicted_y_valid = gradient_descent_algorithm(valid2)
plt.figure()
