...
# 
# Down, because the data was overfit.
...
# Create polynomial features with degree 20
...
# 
# The mean square error decreased compared to the last two models, but this is likely because this model has drastically overfit the data.
...
nDegree = 10 # The degree of the polynomial to fit

### YOUR CODE HERE - Create the polynomial matrix ϕ(X)

# Create ϕ(X) - a matrix with the same number of rows as the training dataset and degree + 1 columns (one column for
# each power of x plus one because of constants). Each cell will be field with the x^i where i is the column of the
# cell and x value of the corresponding row in the training dataset
phi = np.empty((train.shape[0], nDegree + 1))
for row in range(train.shape[0]):
    for degree in range(nDegree + 1):
        xVal = train.iloc[row,0]
        phi[row,degree] = xVal**degree

### YOUR CODE HERE - Find the weighted matrix W

# Find W via W = (X^T * X)^(-1) * X^T * Y
W = np.matmul(np.matmul(np.linalg.inv(np.matmul(phi.T, phi)), phi.T), np.array(train[1]))

### YOUR CODE HERE - Make predictions on the training set and calculate the root mean squared error. Plot the results.

# Find the predicted y values for the training dataset
train_y_poly_pred = np.matmul(W.T, phi.T)

# Calculate root mean squared error
mse = mean_squared_error(train_y, train_y_poly_pred)
print("Mean Squared Error: {}".format(mse))

# Plot the prediction results
plt.scatter(train_x, train_y, s=10)
plt.plot(train_x, train_y_poly_pred, color='r')
plt.show()
...
# Create ϕ(X) for the validation dataset
phi_valid = np.empty((valid.shape[0], nDegree + 1))
for row in range(valid.shape[0]):
    for degree in range(nDegree + 1):
        xVal = valid.iloc[row,0]
        phi_valid[row,degree] = xVal**degree

# Find the predicted y values for the validation dataset
valid_y_poly_pred = np.matmul(W.T, phi_valid.T)
...
plt.scatter(train[0], train[1], s=10)
plt.plot(train[0], train[1], color='r')
plt.show()
