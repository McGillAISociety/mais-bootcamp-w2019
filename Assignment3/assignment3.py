...
# Negate and apply exponential
val = val.mul(-1)
val = torch.pow(np.e, val)

# Sum, multiply by 2 and square
val = (val.sum() * 2)**2 
...
for x_j in val1:
    sum_exp += np.e**x_j
# Softmax
result1 = torch.pow(np.e, val1).mul(1/sum_exp)
...
class NeuralNet(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.input_weights = nn.Parameter(torch.randn(input_dim, hidden_dim))
        self.input_bias = nn.Parameter(torch.randn(hidden_dim))
        
        self.hidden_weights = nn.Parameter(torch.randn(hidden_dim, output_dim))
        self.hidden_bias = nn.Parameter(torch.randn(output_dim))
        
        self.sigmoid = nn.Sigmoid()

    def forward(self, data):
        output_1 = self.sigmoid(torch.matmul(data, self.input_weights) + self.input_bias);
        output_2 = self.sigmoid(torch.matmul(output_1, self.hidden_weights) + self.hidden_bias);
        return output_2
...
#Only trained for an hour, didn't give myself enough time to finish training it.
