...
### YOUR CODE HERE
val1=torch.exp(val1)
result1=torch.sum(val1)
result1=val1/result1
print(result1)
...
### YOUR CODE HERE
trainset=[(torch.tensor(np.asarray(image)/256), torch.tensor(label)) for image,label in trainset]
val_and_test_set=[(torch.tensor(np.asarray(image)/256), torch.tensor(label)) for image,label in val_and_test_set]
# Split the val_and_test_set into two separate datasets 
