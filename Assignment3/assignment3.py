...
### YOUR CODE HERE
print(A+B)               # Equiv to A.mul_(B) but this modifies A. We could also do c = torch.mul(A, B)
...
#np_trainset will be a numpy array: (0 ,1, 2). In 0 we will have our index for a certain image.
# in 1 we will have a 32*32 pixels each represented with a tuple representing pixel RGB values (0-256).
# in 2 we will have a single integer corresponding to class.


#Endgoal is an object (tensor 32 *32 *3, category)


## KK let's try this shit.. SLOWLY. 
...
for i in range(len(trainset)):
    fkme = trainset[i]
    category = fkme[1]
    pixels = np.array(fkme[0].getdata())      # Getdata on the image returns all the RGB for all da pixellz
    pixels = np.multiply(pixels, 1.0/256)     # Normalize (all pixel values for RBG in range 0-1)
    pixels = pixels.flatten()                 # Flatten so it goes all listy (RGBRGBRGB...) instead of tupley (RGB, RGB...)
    fkme_tensor = torch.from_numpy(pixels).float()    # Yeet it to a tensor
    trainset_list.append(tuple((fkme_tensor, category)))

print("WHATTTTTTT THE HELL AM I DOING WITH MY LIFE WHY AM I SO FKING BAD # 1")
...
for i in range(len(valset)):
    ...
    pixels = pixels.flatten()                 # Flatten so it goes all listy (RGBRGBRGB...) instead of tupley (RGB, RGB...)
    fkme_tensor = torch.from_numpy(pixels).float()    # Yeet it to a tensor
    valset_list.append(tuple((fkme_tensor, category)))

print("WHATTTTTTT THE HELL AM I DOING WITH MY LIFE WHY AM I SO FKING BAD # 2")
...
for i in range(len(testset)):
    ...
    pixels = pixels.flatten()                 # Flatten so it goes all listy (RGBRGBRGB...) instead of tupley (RGB, RGB...)
    fkme_tensor = torch.from_numpy(pixels).float()    # Yeet it to a tensor
    testset_list.append(tuple((fkme_tensor, category)))

print("WHATTTTTTT THE HELL AM I DOING WITH MY LIFE WHY AM I SO FKING BAD # 3")




trainloader = torch.utils.data.DataLoader(trainset_list, batch_size=40,shuffle=True, num_workers=2)
valoader = torch.utils.data.DataLoader(valset_list, batch_size=40,shuffle=True, num_workers=2)
testloader = torch.utils.data.DataLoader(testset_list, batch_size=40,shuffle=True, num_workers=2)
...
for epoch in range(EPOCHS):
    ...
    average_loss = float(total_loss) / len(trainloader)
    ...
    with torch.no_grad():
        ...
        correct = 0
        for images2, labels2 in valoader:
            
            net_out = net(images2)
            pred = net_out.data.max(1)[1]  # get the index of the max log-probability
            correct += pred.eq(labels2.data).sum()

        val_acc =  100.0 * float(correct) / len(valoader.dataset)
    
    print("(epoch, train_loss, val_acc) = ({0}, {1}, {2})".format(epoch, average_loss, val_acc))
    
    
    
    # I wonder why validation accuracy is increasing? Data leakage? or are we supposed to do this in this specific Q.
...
with torch.no_grad():
    ...
    test_acc =  100.0 * float(correct) / len(testloader.dataset)
