...
tensor = tf.range(start=0,limit=10,delta=1, dtype=None)
...
### YOUR CODE HERE
numerator = torch.exp(val1)
denom = torch.sum(numerator)
result1 = numerator/denom
...
val_set = [(torch.tensor(image.reshape(1,-1)),torch.tensor(label)) for image, label in val_set]

test_set = [(torch.tensor(image.reshape(1,-1)),torch.tensor(label)) for image, label in test_set]
...
class NeuralNet(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        ### YOUR CODE HERE
        self.iWeight = nn.Parameter(torch.randn(input_dim,hidden_dim).double())
        self.iBias = nn.Parameter(torch.randn(1,hidden_dim).double()) 
        
        self.oWeight = nn.Parameter(torch.randn(hidden_dim, output_dim).double())           
        self.oBias = nn.Parameter(torch.randn(1,output_dim).double())
    ...
    def forward(self, data):
        ...
        ### YOUR CODE HERE
        temp1 = data @ self.iWeight +self.iBias
        temp2 = self.activation(temp1)
        temp3 = temp2 @ self.oWeight + self.oBias
