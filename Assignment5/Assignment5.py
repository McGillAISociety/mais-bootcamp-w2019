...
y_base_pred = predict(model_base, X_test)
print('Your accuracy is:', accuracy_score(y_test, y_base_pred))
...
#     model.add(Conv2D(32, kernel_size=(5, 5), activation='relu',input_shape=IMG_SHAPE_CHANNELS)) # Was 3x3
#     model.add(Conv2D(64, (3, 3), activation='relu'))
#     # Reduce image dimension usin maxpooling (Only keep the max pixel value for every 2x pixel grid)
#     model.add(AveragePooling2D(pool_size=(2, 2))) #Was Maxpooling
#     #The dropout layers help reduce overfitting
#     model.add(Dropout(0.25))
#     model.add(Flatten())
...
#Despite all the layers I tried to add and remove, it seemed like I obtained the best results on the training set
#using the layers of the base model. Adding the Dropout layers allowed me to reduce overfitting and habve better 
#results on the test set. Otherwise, adding convolution layers, changing the pool size, using average pooling
#instead of maxpooling, adding dense layers or changing the activatino function all had adverse effects. 
