...
BATCH_SIZE = 256 # Reduce this to 256, 128, 64, etc if your computer is too slow (512)
...
model_base.fit(x=X_train, y=y_train_enc, epochs=7, batch_size=BATCH_SIZE)
...
def create_model():
    ...
    model.add(Conv2D(96, (11, 11), strides=4, padding='same', activation='relu', input_shape=IMG_SHAPE_CHANNELS))
...
# I first tried to copy Alex Net -- obtained accuraccy of 0.09% 
# Then I asked one of the execs and told me to reduce complexity and achieved 51% accuracy 
# (I was not able to use Google colab so I did all my tests on computer and it took a lot of time) 
# Afterwards, I tried to make the neural network more complex but it gave me 11% accuracy.
# I went back to initial network that gave me 51% (but it is not giving me the same result) Why?
# I looks like I did not run the cells (but this is because I restarted the kernel everytime)
